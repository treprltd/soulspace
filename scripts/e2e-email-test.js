...
// Node.js 22+ ships it natively. On Node.js 20 we fall back to the 'ws' package.
...
const EMAIL_REPORT    = process.env.EMAIL_REPORT_PATH ?? path.join(__dirname, '..', 'e2e-email-report.md')
...
function buildEmailReport() {
  ...
  return `# Soul Space Email Delivery Test Report
Date: ${date}
Target: ${BASE_URL}
...
Generated by scripts/e2e-email-test.js
`
}
...
function printReport() {
  ...
  if (WRITE_REPORT) {
    try {
      fs.writeFileSync(EMAIL_REPORT, buildEmailReport(), 'utf8')
      log(`  Report: ${EMAIL_REPORT}`)
    } catch {}
  }
}
...
async function main() {
  ...
  try {
    ...
    // Write a minimal report so CI can surface the failure
    if (WRITE_REPORT) {
      try {
        const report = `# Soul Space Email Delivery Test Report\nDate: ${new Date().toISOString()}\n\n❌ SETUP FAILED: ${err.message}\n`
        fs.writeFileSync(EMAIL_REPORT, report, 'utf8')
      } catch {}
    }
    process.exit(1)
  }
